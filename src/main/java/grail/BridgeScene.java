...
public class BridgeScene implements BridgeInterface
{
	...
	{
		ArrayList<AvatarInterface> holder=new ArrayList<AvatarInterface>();
		holder.add(arthur);
		holder.add(galahad);
		holder.add(robin);
		holder.add(lancelot);
		...
		{
			holder.get(i).move(holder.get(i).getStartX()-holder.get(i).getHead().getX(), holder.get(i).getStartY()-holder.get(i).getHead().getY());
		}
	}
}
